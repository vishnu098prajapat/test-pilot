 ...
 *
 * - suggestTopics - A function that takes a subject string and returns topic suggestions.
...
const SuggestTopicsInputSchema = z.object({
  subject: z.string().min(1, "Subject cannot be empty.").describe("The general subject for which topics are to be suggested (e.g., 'Mathematics', 'History', 'Python'). This should be a single subject."),
});
...
Based on the single subject provided in "{{subject}}", please suggest 5-7 relevant topics or sub-topics that would be suitable for generating test questions for a general audience (e.g., high school or early college level).

Focus on core concepts and commonly taught areas within the specified subject.

Return your response as a JSON object with a single key "topicSuggestions", where the value is an array of strings. For example, if the subject is "Physics":
{
  "topicSuggestions": ["Newton's Laws of Motion", "Thermodynamics", "Optics", "Electromagnetism", "Quantum Mechanics basics"]
}
