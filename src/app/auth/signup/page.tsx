...
export default function SignupPage() {
  ...
  const { login } = useAuth(); // Using login from useAuth for consistency after Google sign-in
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleGoogleSignUp = async () => {
    setIsSubmitting(true);
    try {
      const result = await signInWithGoogleAction(); // Google Sign-In handles both signup and login
      if (result.success && result.user) {
        login(result.user); // Update client-side auth state and localStorage
        toast({
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-background to-secondary p-4">
      ...
      <Card className="w-full max-w-md shadow-xl">
        <CardHeader>
          ...
          <CardDescription className="text-center">Join Test Pilot using Google.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <Button onClick={handleGoogleSignUp} className="w-full" disabled={isSubmitting}>
